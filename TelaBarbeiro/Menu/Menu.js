...
import relatorio4 from "../../assets/relatorio4.png";
...
const Menu = () => {
        ...
        const getCurrentUser = async () => {
          ...
          if (currentUser) {
            ...
            try {
              ...
              const userRef = ref(database, `users/${currentUser.uid}/nomeBarbearia`);  // Buscar nomeBarbearia
              const snapshot = await get(userRef);
              console.log('Snapshot.exists():', snapshot.exists());
              console.log('Snapshot.val():', snapshot.val());
    
              if (snapshot.exists()) {
                const barberNameFromDB = snapshot.val();
                setBarberName(barberNameFromDB);  // Alterado para setBarberName
              } else {
                console.log('Nome da barbearia não encontrado no banco de dados.');
              }
            } catch (error) {
              console.error('Erro ao buscar o nome da barbearia:', error);
            }
          } else {
    ...
    function MyCalendar() {
        return navigator.navigate("MyCalendar")
        function Home() {
            return navigator.navigate("Home")
        }
        function Rotas() {
            return navigator.navigate("Rotas")
        }
    }
        ...
        <View >
            ...
            <View style={estilo.fundo}>
                ...
                <View style={estilo.Div1}>
                    <TouchableOpacity style={estilo.btn1} onPress={() => navigator.navigate('Rotas')}>
                        <Image source={relatorio4} style={estilo.icons} />
                        <Text style={estilo.text_button}>RELATÓRIO</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={estilo.btn2} onPress={() => navigator.navigate('MyCalendar')}>
                        <Image source={calendar1} style={estilo.icons} />
                        <Text style={estilo.text_button}>AGENDA</Text>
                    </TouchableOpacity>
                </View>

                <View style={estilo.Div2}>
                    <TouchableOpacity style={estilo.btn2} onPress={() => navigator.navigate('Barber')}>
                        <Image source={barber} style={estilo.icons} />
                        <Text style={estilo.text_button}>BARBEARIA</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={estilo.btn1} onPress={() => navigator.navigate('Home')}>
                        ...
                        <View style={{ backgroundColor: '#14213D', width: 300, height: 842, borderTopRightRadius: 50 }}>
                            <View style={{ backgroundColor: "orange", borderRadius: 20, borderTopLeftRadius: 0, borderTopRightRadius: 50, borderBottomLeftRadius: 50, borderBottomRightRadius: 0, }}>
                                <View style={{ alignItems: 'center', top: 20, }}>
                                    ...
                                    <Text style={{ padding: 60, textAlign: "center", fontSize: 20, fontFamily: "MontBold", bottom: 10 }}>{barberName}</Text>
                                </View>
                            </View>
